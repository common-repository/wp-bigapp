...
class BigAppAdmin{
    ...
    public static function display_main(){
        $valid_actions = array('main','menu','banner','sepcail','extend','mobile_page');
        ...
        case "banner":
            ...
            break;
		case "sepcail":
            self::display_bigapp_admin_special_page();
            break;
		 case "extend":
    ...
    public static function display_bigapp_admin_main_page(){
        //plugin base info
        $base_info = BigAppAdminModel::get_plugin_base_info(true);
        //菜单管理模块信息
     ...
     * 生成专题的管理页面
     */
    public static function display_bigapp_admin_special_page(){
        $menu_id = 0;
        if (isset($_REQUEST['menu_id'])){
            $menu_id = $_REQUEST['menu_id'];
        }
	...
	public static function display_bigapp_admin_extend_page(){
        ...
        $page_data = BigAppAdminModel::get_plugin_base_info();
     ...
     * 插件列表添加设置链接
     */
    public static function wp_bigapp_plugin_action_links( $links, $file ) {
        $plugin_flag = "wp-bigapp/bigapp.php";
        if ( $file != $plugin_flag)
            return $links;
        $settings_link = '<a href="admin.php?page=bigapp_admin">' . __( '设置', 'setting' ) . '</a>';
