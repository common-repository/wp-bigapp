    ...
    Uz = function () {
        ...
        var setMenu = function(v,url){
            ...
            function getConfItem(val){
                var setturl = '<td><a href="'+v.menu_conf.opt_url+'&menu_id='+val.ID+'&menu_name='+encodeURIComponent(val.name)+'" class="button">内容设置</a></td>';
                if (val.type == 'custom' || val.type == 'post_type'){
                    setturl = '<td></td>';
                }
                var str = ''+
                    '<tr class="conf-item">'+
