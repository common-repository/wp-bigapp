...
class bigapp_server_api {
    ...
    public function get_base_conf(){
        ...
        $conf['bigapp_special_switch'] = "1";
		$conf['bigapp_special_list_style'] = "1";
		$conf['bigapp_special_detail_style'] = "1";
        if($special){
            $special_conf = json_decode(get_option('bigapp_special_conf'),true);
            $conf['bigapp_special_switch'] = $special_conf['switch'];
			$conf['bigapp_special_list_style'] = $special_conf['list_style'];
			$conf['bigapp_special_detail_style'] = $special_conf['detail_style'];
        }
        return $conf;
    }
	...
	public function get_special_post_list($id,$page=1,$filter= array()){
		...
		$response->data= sort_by_key($response->data,'bigapp_special_order');
		if(isset($page) && isset($filter['pre_page'])){
			$response->data = get_list_pagination($response->data,$page,$filter['pre_page']);
		}
		return $response;
	}
	
	public function gen_image(){
		echo gen_new_img('http://192.168.180.93:8080/wordpress/wp-content/uploads/2015/09/DSC2711-300x169.jpg');
	}
