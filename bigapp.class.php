...
class mobileplugin_bigapp {
    ...
    public static function init(){
		...
		self::plugin_init_bigapp_special();
    }
	...
	public static function plugin_init_bigapp_special(){
			register_taxonomy(   
			'bigapp_special',   
			array('post','page'),   
			array(   
				'hierarchical' => true,   
				'labels' => array("bigapp_special"),   
				'show_ui' => false,   
				'query_var' => true,   
				'rewrite' => array( 'slug' => 'bigapp_special_slug' ),   
			)   
		);
	}
    ...
    public static function bigapp_auth_login($redirect_to, $requested_redirect_to, $user){
		...
		if(isset($_GET[$route_prefix]) && $_GET[$route_prefix] == 'sociallogin'){
            bigapp_core::yz_auth_social_login();
        }
        ...
        if ( !is_wp_error($user) ){
            ...
            $result['data'] = array(
                ...
                'id' => $user->ID,
                'nice_name' => urldecode($user->user_nicename),
                'email' => $user->user_email,
                'reg_time' => strtotime($user->user_registered),
                'status' => $user->user_status,
                'description' => $user->description,
                'display_name' => $user->display_name,
                'avatar' => json_get_avatar_url($user->user_email?$user->user_email:$user->ID),
                'roles' => $user->roles,
				'logout_url' => wp_create_nonce('logout'),
            );
			$result['data']['hasbind'] = bigapp_core::yz_auth_social_bind($user->ID);
        }else{
    ...
    /**
     * 主要是除去current_action 这个函数要求版本在3.9以上
     */
    public static function bigapp_json_cookie_collect_status_valid() {
        global $wp_json_auth_cookie;
