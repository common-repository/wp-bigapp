...
define( 'YZ_BIG_INNER_VERSION', '10830' );
define( 'YZ_BIG_LAST_MODIFICATION', ' 2016-03-16 17:28' );
define( 'YZ_APP_API_VERSION', '1' );
define( 'YZ_APP_MINIMUM_WP_VERSION','3.4' );
...
function json_api_wp_version_warning($plugin_meta, $plugin_file, $plugin_data, $status) {
    if(version_compare($GLOBALS['wp_version'], YZ_APP_MINIMUM_WP_VERSION, '<') && strpos($plugin_file, 'bigapp') !== false){
        $plugin_meta[] = '<font style="line-height:30px;color:red;font-weight:bold">Sorry, YZ Json API requires WordPress version ' . YZ_APP_MINIMUM_WP_VERSION . ' or greater.</font>';
    }
    return $plugin_meta;
}
...
add_action( 'auth_cookie_valid',        array('mobileplugin_bigapp','bigapp_json_cookie_collect_status_valid' ) );
//头像
...
if ( is_admin() ) {  
	...
	add_filter( 'plugin_action_links', array('BigAppAdmin','wp_bigapp_plugin_action_links'),10,2);    //添加设置到列表
}
