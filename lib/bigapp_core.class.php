...
class bigapp_core{
	...
	public static function yz_auth_social_login(){
		...
		if(empty($uid[0])){
			...
			$result['avatar'] = json_get_avatar_url($userInfo[0]->user_email?$userInfo[0]->user_email:$userInfo[0]->ID);
	...
	public static function yz_auth_social_bind($uid){
		...
		$bind = isset($_POST['bind'])?$_POST['bind']:false;
    ...
    public static function set_response($result=null){
        if(empty($result)){
            ...
            if(isset($result['error_msg']) && is_string($result['error_msg'])){
                $result['error_msg'] = __lan($result['error_msg']);
            }
        }
