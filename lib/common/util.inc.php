        ...
        case 3:
            ...
            if($rewrite != false){   //自定义链接 
                ...
                if($matches){
                    ...
                    $filter["filter[only_one]"] = 1;        //主要是banner区的链接指向一个文章，没有考虑到文章列表的情况
 ...
 * 压缩图片
 */
function get_resize_img($path,$quality = 10 ,$sizes=array()){
	$dirInfo = wp_upload_dir();
	if($dirInfo['error'] != false){
        return $path;
    }
	...
	if ( ! is_wp_error( $image ) ) {
		  ...
		  if( isset($sizes['width']) && isset($sizes['height']) ){
			$image->resize( $sizes['width'], $sizes['height'], true );  
		  }
		  $file_ext = strtolower( pathinfo( $path, PATHINFO_EXTENSION ) );
		  $file = '/img_' . md5($path) . '.' . $file_ext;
		  $fullPath = $dirInfo['path'].$file;
		  if(!file_exists($fullPath)){
			  $image->save( $fullPath );
		  }
		  $path = $dirInfo['url'].$file;
	}
	return $path;
}

function gen_new_img($url){
	$mod = isset($_GET['img_mod'])?intval($_GET['img_mod']):2;;
	$modMap = array(0=>100,1=>0,2=>10,3=>100);
	$quality = isset($modMap[$mod])?$modMap[$mod]:100;
	if(2 == $mod){
		return get_resize_img($url,$quality);
	}
	else{
		return $url;
	}
...
function get_list_pagination($list,$page=1,$limit = 20){
	$page = intval($page) > 0 ? intval($page) : 1;
	$limit = intval($limit) > 0 ? intval($limit) : 20;
	$start = ($page-1) * $limit;
	if( count($list) >= $page * $limit){
		$ret = array_slice($list,$start,$limit);
	}else{
		if($page == 1){
			$ret = $list;
		}else{
