...
class WP_JSON_Taxonomies {
	...
	public function register_routes( $routes ) {
		$tax_routes = array(
            $this->route=> array(
                ...
                "get_post_tags" => array( array( $this, 'get_post_tags' ), WP_JSON_Server::READABLE ),
				"get_post_bigapp_special" => array( array( $this, 'get_post_bigapp_special' ), WP_JSON_Server::READABLE ),
			),
		);
    ...
    public function get_nav_list($context='view'){
        ...
        if($switch == 1){
            ...
            foreach($menu_list as &$list){
                if($list['type'] == 'custom' && preg_match("@".get_bloginfo('siteurl')."@i",$list['link'])){     //对于内部的文章地址
                    $list['link_org'] = $list['link'];
                    $list['link'] = convert_link2json($list['link'],2);
                    $list['type'] = 'post_type';
                }
    ...
    public function get_post_tags($filter= array()){
        ...
        $filter['order'] = "desc";
        $filter['number'] = 5;      // 只返回前5个
        $lists = $this->get_taxonomy_terms('post_tag',$filter);
        return $lists;
    }
     ...
     * 获取专题列表
     */
	public function get_post_bigapp_special($filter= array(),$page=1){
        $ret = $this->get_taxonomy_terms('bigapp_special',$filter);
		
		if(!empty($ret))
		foreach($ret as &$r){
			if(!empty($r['description'])){
				$des = json_decode($r['description'],true);
				$r = array_merge($r,$des);
			}
			$r['link'] = json_url('bigapp_api','get_special_post_list',array('id'=>$r['ID']));
			if(!isset($r['date'])){
				$r['date'] = date("Y-m-d H:i:s");
			}
			unset($r['description']);
		}
		$ret = sort_by_key($ret,'order');
		if(isset($page) && isset($filter['pre_page'])){
			$ret = get_list_pagination($ret,$page,$filter['pre_page']);
		}
		return $ret;
    }
	...
	public function get_taxonomy_terms( $taxonomy, $filter = array() ) {
		if ( ! taxonomy_exists( $taxonomy ) ) {
            json_error(BigAppErr::$taxonomy['code'],"Invalid taxonomy ID.");
		}

		$args = array(
			'hide_empty' => false,       //隐藏没有被关联的tag
		);
		...
		if ( is_wp_error( $terms ) ) {
            json_error(BigAppErr::$taxonomy['code'],"get_term return error");
		}
	...
	public function add_term_data( $data, $post, $context ) {
		...
		$data['terms']['post_tag'] = array();   //这边特定加了post_tag，为了前端对象解析
