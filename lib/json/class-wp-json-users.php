...
class WP_JSON_Users {
	...
	public function get_user( $id, $context = 'view' ) {
		...
		if ( $current_user_id !== $id && ! current_user_can( 'list_users' ) ) {
            json_error(BigAppErr::$user['code'],"Sorry, you are not allowed to view this user.");
		}

		$user = get_userdata( $id );

		if ( empty( $user->ID ) ) {
            json_error(BigAppErr::$user['code'],"Invalid user ID .");
		}
	...
	protected function insert_user( $data ) {
		...
		if ( ! empty( $data['ID'] ) ) {
			...
			if ( ! $existing ) {
                json_error(BigAppErr::$user['code'],"Invalid user ID",$data['ID']);
			}

			if ( ! current_user_can( 'edit_user', $data['ID'] ) ) {
                json_error(BigAppErr::$user['code'],"Sorry, you are not allowed to edit users.");
			}
			...
			if ( ! current_user_can( 'create_users' ) ) {
                json_error(BigAppErr::$user['code'],"Sorry, you are not allowed to create users.");
			}
			...
			foreach ( $required as $arg ) {
				if ( empty( $data[ $arg ] ) ) {
                    json_error(BigAppErr::$user['code'],"create user,missing parameter ",$arg);
				}
			}
		...
		if ( ! empty( $data['URL'] ) ) {
			...
			if ( $escaped !== $user->user_url ) {
                json_error(BigAppErr::$user['code'],"Invalid user URL.");
			}
	...
	public function edit_user( $id=0, $data= array() ) {
		...
		if ( empty( $id ) ) {
            json_error(BigAppErr::$user['code'],"User ID must be supplied.");
		}

		// Permissions check
		if ( ! current_user_can( 'edit_user', $id ) ) {
            json_error(BigAppErr::$user['code'],"Sorry, you are not allowed to edit this user.");
		}

		$user = get_userdata( $id );
		
		if ( ! $user ) {
            json_error(BigAppErr::$user['code'],"User ID is invalid.");
		}
		...
		if ( is_wp_error( $retval ) ) {
            json_error(BigAppErr::$user['code'],"Update user info faild.",json_encode($data));
		}
    ...
    public function upload_avatar(){
        ...
        if(get_option('wp_user_avatar_allow_upload',0) == 0){       //是否开启头像上传
            json_error(BigAppErr::$user['code'],'cant allow to upload avatars');
        }
        if(!($user_id = get_current_user_id())){
            json_error(BigAppErr::$user['code'],'need login');
        }
        // wp_handle_upload
        ...
        if(!empty($_FILES['wpua-file'])) {
            ...
            if(is_writeable($upload_dir['path'])) {
                if(!empty($type) && preg_match('/(jpe?g|gif|png)$/i', $type)) {
                    ...
                    if(isset($file['error'])){
                        json_error(BigAppErr::$user['code'],'upload file error',$file['error']);
                    }
                    ...
                    json_error(BigAppErr::$user['code'],'file type error');
                }
            }else{
                json_error(BigAppErr::$user['code'],'upload file path cant write');
            }
        }else{
            json_error(BigAppErr::$user['code'],'empty file');
        }
        return $url;
    }
    /**
    ...
    public function get_user_info_from_post(){
        ...
        if(isset($_POST['password'])){
            ...
            if(strlen($user_info['password']) < 4){
                json_error(BigAppErr::$user['code'],'password length too short!');
            }
        }
        if(isset($_POST['description'])){
            $user_info['description'] = $_POST['description'];
        }
